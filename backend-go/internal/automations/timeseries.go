...
func StartTimeSeriesUpdates(instance *blockchain.Blockchain) { 
	...
	const windowSize = 30 
	var bloodData, tempData, heartData, spO2Data []blockchain.DataStructsDataPoint
	...
	for range ticker.C {
		...
		newTempPoint := blockchain.DataStructsDataPoint{Timestamp: now, Value: big.NewInt(int64(36 + rand.Intn(2))), Change: big.NewInt(0)}
		...
		queueReplaceRequest(instance, "bloodStatus", bloodData)
		queueReplaceRequest(instance, "temperature", tempData)
		queueReplaceRequest(instance, "heartRate", heartData)
		queueReplaceRequest(instance, "spO2", spO2Data)
