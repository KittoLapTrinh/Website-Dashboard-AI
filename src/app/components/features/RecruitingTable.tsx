...
const recruitingData = [
  {
    icon: <SiTesla size={20} color='#484A52' />, // Biểu tượng cho foundation
    foundation: 'FPT',
  ...
  {
    icon: <SiApple size={20} color='#484A52'/>, // Bạn có thể thay bằng icon Google
    foundation: 'Google',
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-6 gap-x-4 px-4 pb-3 border-b border-white-800 ">
            {['Foundation', 'Job Position', 'Field', 'Salary', 'Form', 'Update'].map((header) => (
              <div key={header} className="text-2sm text-white font-semibold flex items-center gap-1 cursor-pointer hover:text-white">
                {header}
                <ArrowUpDown size={12} />
              </div>
            ))}
